...
public class IngresTransactionMonitor extends TransactionMonitor {
    ...
    private Struct adjustTxId(Struct txStruct, OffsetContext offsetContext) {
        final String commitRecord = ((IngresOffsetContext) offsetContext).getChangePosition().getCommitRecord().toBase64();
        final String txId = offsetContext.getTransactionContext().getTransactionId();
        txStruct.put(DEBEZIUM_TRANSACTION_ID_KEY, String.format("%s:%s", txId, commitRecord));
        return txStruct;
    }
}
