...
/**
 * Integration test for the user-facing history topic of the Debezium Ingres Server connector.
 * <p>
...
public class SchemaHistoryTopicIT extends AbstractAsyncEngineConnectorTest {
    ...
    public void snapshotSchemaChanges() throws Exception {
        ...
        assertThat(records.recordsForTopic(TestHelper.topicName("tablea"))).hasSize(RECORDS_PER_TABLE);
        assertThat(records.recordsForTopic(TestHelper.topicName("tableb"))).hasSize(RECORDS_PER_TABLE);
        records.recordsForTopic(TestHelper.topicName("tableb")).forEach(record -> assertSchemaMatchesStruct(
                (Struct) ((Struct) record.value()).get("after"),
                SchemaBuilder.struct()
                        .optional()
                        .name(TestHelper.getDBPrefix() + "tableb.Value")
