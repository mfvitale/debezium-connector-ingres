...
/**
 * Transaction metadata test for the Debezium Ingres Server connector.
 *
 */
public class TransactionMetadataIT extends AbstractAsyncEngineConnectorTest {
    ...
    public void transactionMetadata() throws Exception {
        ...
        final Configuration config = TestHelper.defaultConfig()
                ...
                .with(IngresConnectorConfig.PROVIDE_TRANSACTION_METADATA, true)
                .build();
        ...
        final List<SourceRecord> tx = records.recordsForTopic(TestHelper.getDBName() + ".transaction");
        ...
        assertEndTransaction(all.get(2 * RECORDS_PER_TABLE + 1), txId, 2 * RECORDS_PER_TABLE,
                Collect.hashMapOf(TestHelper.topicName("tablea"), RECORDS_PER_TABLE, TestHelper.topicName("tableb"), RECORDS_PER_TABLE));
    }
    ...
    protected void assertEndTransaction(SourceRecord record, String beginTxId, long expectedEventCount, Map<String, Number> expectedPerTableCount) {
        ...
        final String expectedTxId = String.format("%s:%s", expectedId, offset.get(SourceInfo.COMMIT_HEADER));
